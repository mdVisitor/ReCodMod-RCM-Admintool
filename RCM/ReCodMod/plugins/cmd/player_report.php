...
if ($x_stop_lp == 0)
			...
			if ($stime - $cron_time >= 60 * 3)
				...
				try
					...
					if ($stat > 0)
						...
						if ($guids == 0) $db5->exec("INSERT INTO playerlist ( idnum, name, ip, ping ) VALUES ('0', '$nickr', '$date', '0');");
						  else $db5->exec("INSERT INTO playerlist ( idnum, name, ip, ping ) VALUES ('0', '$guidn', '$date', '0');");
						}
					}
										...
										if ($x_stop_lp == 0)
											...
											if (strpos($admin_email, ';') !== false)
												...
												while ($x++ < $countnumbs)
													...
													*/
													$msg = 'Nickname: ' . repooort($nickrn) . ' Message: ' . repooort($msgrn) . ' UserIp: ' . $iprepp . ' ServerIp: ' . repooort($servernamex) . ':' . $server_port;
													if (!empty($webhookurl))
														{
														if (!empty($msg))
															{
	...
	if ($game_patch != 'cod1_1.1')
		...
		if (time() - $cron_timeq >= 60 * $players_access_xget)
			{
			if ((strpos($msgr, 'xget ') !== false) && ($x_number != 1) || (strpos($msgr, 'xgetss ') !== false))
				{
